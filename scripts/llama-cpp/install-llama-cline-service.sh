...
# This service is intentionally manual-start. Keep it disabled even if it was
# accidentally enabled before.
sudo systemctl disable "$UNIT_NAME" >/dev/null 2>&1 || true

echo "Installed ${UNIT_DST}"
echo
echo "Manual controls:"
echo "  sudo systemctl start llama-cline"
echo "  sudo systemctl status llama-cline"
echo "  sudo systemctl stop llama-cline"
echo "  journalctl -u llama-cline -f"
